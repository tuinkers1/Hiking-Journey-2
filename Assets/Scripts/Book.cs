...
public class Book : MonoBehaviour
{
  ...
  [SerializeField] List<Transform> pages;
    ...
    public void ForwardButtonActions()
    {
        ...
        {
            backButton.SetActive(true); // every time we turn the page forward, the back button is activated
        }
    ...
    IEnumerator Rotate(float angle, bool forward)
    {
        ...
        while (true) 
        {
            ...
            float angle1 = Quaternion.Angle(pages[index].rotation, targetRotation); //calculate the angle between given and current angle of rotation
            if (angle1 < 0.1f)
            {
                if (forward == false)
                {
